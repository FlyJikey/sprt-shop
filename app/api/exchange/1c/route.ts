...
export async function POST(req: NextRequest) {
  ...
  if (mode === 'file') {
    try {
      ...
      const isOffers = filename.includes('offers') || result?.КоммерческаяИнформация?.ПакетПредложений || result?.КоммерческаяИнформация?.ИзмененияПакетаПредложений;
...
async function processOffersFile(json: any) {
  // Пытаемся найти предложения либо в "ПакетПредложений", либо в "ИзмененияПакетаПредложений" (когда выгружаются только остатки 1С)
  const offersPackage = json?.КоммерческаяИнформация?.ПакетПредложений || json?.КоммерческаяИнформация?.ИзмененияПакетаПредложений;
  const rawOffers = offersPackage?.Предложения?.Предложение;

  if (!rawOffers) {
    console.log('[1C] ⚠️ Предложения не найдены в файле offers (ни в ПакетПредложений, ни в ИзмененияПакетаПредложений).');
    return;
  }
